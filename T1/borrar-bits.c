...
uint borrar_bits(uint x, uint pat, int len) {
  uint r;  //variable auxiliar cualquiera
  ...
  while (lo <= a){  //while para recorrer todos los bits de x
    uint r2 = (((x^pat)<<(a-lo))>>(a-len));  //x^pat ve si pat esta en cierto grupo de bits de x y con <<, >> se ignoran bits no evaluados en este ciclo
